...
export function VideoChapterTimeline({ chapters = [] }) {
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-black/20 p-6">
      <div>
        <h3 className="text-xl font-semibold text-indigo-200">影片內容脈絡</h3>
        <p className="mt-1 text-base text-white/45">根據字幕分析影片章節，協助解讀留言討論背景</p>
      </div>
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <h4 className="mt-1 text-lg font-semibold text-white">{chapter.title}</h4>
              </div>
            ...
            {chapter.summary && (
              <p className="mt-3 text-base leading-7 text-white/75">{chapter.summary}</p>
            )}
            ...
            {chapter.keywords.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {chapter.keywords.slice(0, 6).map((keyword) => (
                  <span
                    key={keyword}
                    className="rounded-full bg-white/8 px-2.5 py-1 text-sm text-white/65"
                  >
                    #{keyword}
                  </span>
                ))}
